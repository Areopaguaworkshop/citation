...
class VerticalCitationExtraction(dspy.Signature):
    ...
    title = dspy.OutputField(desc="ACTUAL book title (not series name) - look for smaller text that represents the specific work being cited, like 四書章句集註")
    series = dspy.OutputField(desc="Series or collection name - usually prominently displayed large text like 新編諸子集成")
    ...
    volume = dspy.OutputField(desc="Volume number within series (e.g., 第一輯, 第三冊)")
...
class VerticalCitationLLM(CitationLLM):
    ...
    def parse_multiple_authors(self, author_string: str) -> List[Dict]:
        ...
        for part in author_parts:
            ...
            # Add dynasty and role as separate fields if needed
            if dynasty:
                author_info['dynasty'] = dynasty
            if role:
